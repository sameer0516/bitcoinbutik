'use client';
// app/collections/rings/[ringsSlug]/RingsDetailClient.jsx
// ✅ FIX: Fallback fetch mein p.slug se match karo — createSlug(title) nahi
...
const formatProductData = (productData) => {
    ...
    return {
        ...
        slug: productData.slug, // ✅ slug preserve karo
...
export default function RingsDetailClient({ ringsSlug, initialProduct }) {
    const router = useRouter();
    const { addToCart } = useCart();

    const [product, setProduct] = useState(initialProduct ? formatProductData(initialProduct) : null);
    ...
    // ─── Fetch Product (fallback — only if initialProduct nahi mila) ──────────

    useEffect(() => {
        if (initialProduct) return; // server ne diya hai — fetch mat karo
        if (!ringsSlug) return;
        window.scrollTo(0, 0);
        ...
        const fetchProduct = async () => {
            ...
            try {
                // Session/local cache check
                const sessionKey = `product_${ringsSlug}`;
                let passedData = null;
                ...
                // ✅ FIX: API se fetch karo aur p.slug se match karo
                // createSlug(title) se match karna GALAT tha — slug mismatch hota tha
                const response = await fetch(`${API_URL}/api/products?category=Rings`);
                if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);

                const allProducts = await response.json();

                // ✅ FIXED MATCH: p.slug directly — no createSlug
                const foundProduct = allProducts.find((p) => p.slug === ringsSlug);

                if (!foundProduct) throw new Error(`Product not found for slug: "${ringsSlug}"`);
        ...
        fetchProduct();
    }, [ringsSlug, initialProduct, fetchLatestStock]);

    // ─── Stock Polling (every 5s) ─────────────────────────────────────────────

    useEffect(() => {
        if (!product?.id) return;
        const interval = setInterval(() => fetchLatestStock(product.id), 5000);
    ...
    useEffect(() => { setCurrentMediaIndex(0); }, [ringsSlug]);
    ...
    const handleAddToCart = async () => {
        ...
        const result = await addToCart(cartItem, selectedSize, quantity);
                    ...
                    <div className="tab-pane">
                        ...
                        <ul>
                            <li className="detail-item">
                                <strong>Type:</strong> Rings <span>{product.type}</span>
                            </li>
                            <li className="detail-item">
                        ...
                        <div className="pdp-details-container">
                            ...
                            <div className="pdp-options-grid">
                                <div className="pdp-option-group">
                                    <label className="pdp-option-label" htmlFor="size-select">Ring Size:</label>
                                    <select
                                        id="size-select"
